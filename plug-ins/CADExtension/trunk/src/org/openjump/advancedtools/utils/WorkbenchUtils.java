...
public class WorkbenchUtils {
     ...
     */
    public static Layer addMemoryLayerToOpenJUMP(String categoryName,
            String layerName, FeatureCollection fc) {
        Layer layer = JUMPWorkbench.getInstance().getFrame().getContext()
                .getLayerViewPanel().getLayerManager()
    ...
    public static List<Feature> getSelectedFeatures() {
        ...
        if (layerViewPanel != null) {
            SelectionManager selectionManager = layerViewPanel
                    .getSelectionManager();
            Collection<Feature> featuresWithSelectedItems = selectionManager
    ...
    /**
     * gets the selected features in the active view and given layer of
     * OpenJUMP, if any. Never returns null.
     * 
     * @return
     */
    ...
    /**
     * gets the layer that has selected features, if features of more than one
     * layer are selected, returns null
     * 
     * @return the layer that has selected features, if features of more than
     *         one layer are selected, returns null
     */
    public static Layer getSelectedFeaturesLayer() {
        ...
        if (layerViewPanel != null) {
            SelectionManager selectionManager = layerViewPanel
                    .getSelectionManager();
            Collection<Layer> layersWithSelectedItems = selectionManager
     ...
     * 
     * @return Collection<Category>, may be an empty collection but never
     *         returns null.
     */
    ...
    public static double getPixelSize() {
        ...
        if (layerViewPanel != null) {
            ...
            Envelope worldEnvelope = layerViewPanel.getViewport()
    ...
    /**
     * Executes an undoable command add new features. Original features will not
     * be deleted.
     * 
     * @param name
     * @param selection
    ...
    public static void executeAddNewFeaturesRemoveOldFeatures(String name,
            SelectionManager selection, Layer layer,
            List<Feature> featuresToAdd, List<Feature> featuresOriginal)
            throws Exception {
        executeUndoableChanges(name, selection, layer, featuresToAdd,
                featuresOriginal);
    }
     ...
     * @throws Exception
     */
    public static void executeAddNewFeaturesLeaveOldFeatures(String name,
            SelectionManager selection, Layer layer,
            List<Feature> featuresToAdd, List<Feature> featuresOriginal)
            throws Exception {
        executeUndoableChanges(name, selection, layer, featuresToAdd,
    ...
    /**
     * executes an undoable command, a layer modification that can be undone,
     * this call is too clomplex and must be protected. <br>
     * If you need to add a new single feature, or anything less complicated
     * create a new entry with a simpler interface and use this one to implement
     * it if possible
     * 
     ...
     * @throws Exception
     */
    protected static void executeUndoableChanges(String name,
            final SelectionManager selectionManager, final Layer editableLayer,
            final List<Feature> toAdd, final List<Feature> toDelete)
            throws Exception {
        WorkbenchContext context = JUMPWorkbench.getInstance().getFrame()
                .getContext();
        final boolean selection = PersistentBlackboardPlugIn.get(
                context.getLayerViewPanel().getWorkBenchFrame().getContext())
                    ...
                    try {
                        if (!toAdd.isEmpty()) {
                            editableLayer.getFeatureCollectionWrapper()
                                    .removeAll(toDelete);
                            editableLayer.getFeatureCollectionWrapper().addAll(
                                    toAdd);
                        }
                        if (selection) {
    ...
    /**
     * It executes an undoable command: it create new features from selected
     * ones saving the latter
     * 
     * @param selectionManager
     * @param editableLayer
     * @param original
     ...
     * @throws Exception
     */
    public static void executeUndoableAddNewFeatsLeaveSelectedFeats(
            String name, final SelectionManager selectionManager,
            final Layer editableLayer, final Collection<Feature> featureCopies,
            final Collection<Feature> selectedFeatures) throws Exception {
        WorkbenchContext context = JUMPWorkbench.getInstance().getFrame()
                .getContext();
        final boolean selection = PersistentBlackboardPlugIn.get(
                context.getLayerViewPanel().getWorkBenchFrame().getContext())
     ...
     * @throws Exception
     */
    public static void executeUndoableAddNewFeatsRemoveSelectedFeats(
            String name, final SelectionManager selectionManager,
            final Layer editableLayer, final Collection<Feature> toAdd,
            final Collection<Feature> toDelete) throws Exception {
        WorkbenchContext context = JUMPWorkbench.getInstance().getFrame()
                .getContext();
        final boolean selection = PersistentBlackboardPlugIn.get(
                context.getLayerViewPanel().getWorkBenchFrame().getContext())
     ...
     * @throws Exception
     */
    public static void executeUndoableAddNewFeats(String name,
            final SelectionManager selectionManager, final Layer editableLayer,
            final Collection<Feature> toAdd) throws Exception {
        WorkbenchContext context = JUMPWorkbench.getInstance().getFrame()
                .getContext();
        final boolean selection = PersistentBlackboardPlugIn.get(
                context.getLayerViewPanel().getWorkBenchFrame().getContext())
     ...
     */
    public static void zoom(Envelope env)
            throws NoninvertibleTransformException {
        WorkbenchContext context = JUMPWorkbench.getInstance().getFrame()
                .getContext();
        LayerViewPanel layerViewPanel = context.getLayerViewPanel();
    ...
    public static void zoomFeatures(List<Feature> features)
            throws NoninvertibleTransformException {
        Envelope env = null;
     ...
     * @throws NoninvertibleTransformException
     */
    public static void zoomFeatures(List<Feature> features, double margin)
            throws NoninvertibleTransformException {
        Envelope env = null;
        for (Feature feature : features) {
            Geometry geometry = feature.getGeometry();
    ...
    public static void zoomToSelectedFeatures() throws Exception {
        ...
        zoom.execute(JUMPWorkbench.getInstance().getContext()
                .createPlugInContext());
    }
    ...
    public static void zoomToCoordinates(Coordinate[] coord)
            throws NoninvertibleTransformException {
    ...
    public static List<Layer> getActiveViewLayers() {
        return JUMPWorkbench.getInstance().getFrame().getContext()
                .getLayerManager().getLayers();
    }

    public static List<Layer> getActiveNotRasterLayers() {
        ArrayList<Layer> layers = new ArrayList<Layer>();
        List<Layer> some = getActiveViewLayers();
    ...
    public static CursorTool getLastNotDelegatingCursorTool(CursorTool ct) {
        ...
        if (ct instanceof DelegatingTool) {
            return getLastNotDelegatingCursorTool(((DelegatingTool) ct)
                    .getDelegate());
        } else if (ct instanceof LeftClickFilter) {
            return getLastNotDelegatingCursorTool(((LeftClickFilter) ct)
                    .getWrappee());
        } else {
            return ct;
        }
    ...
    /**
     * Does not invalidate the prior selection (features are added to the group
     * of selected features).
     */
    public static void selectFeaturesInLayer(Layer layer,
    ...
    public static QuasimodeTool addStandardQuasimodes(CursorTool tool) {
        ...
        quasimodeTool.add(new ModifierKeySpec(false, false, true),
                new ZoomTool());
        quasimodeTool
                .add(new ModifierKeySpec(false, true, true), new PanTool());
    ...
    public static CursorTool getWrappedTool(CursorTool currentCursorTool) {
        ...
        if (currentCursorTool instanceof QuasimodeTool) {
            CursorTool defaultCursorTool = ((QuasimodeTool) currentCursorTool)
                    .getDefaultTool();
            if (defaultCursorTool instanceof LeftClickFilter) {
                CursorTool wrappeeCursorTool = ((LeftClickFilter) defaultCursorTool)
                        .getWrappee();

                if (wrappeeCursorTool instanceof DelegatingTool) {
                    wrappeeCursorTool = ((DelegatingTool) wrappeeCursorTool)
                            .getDelegate();
                }
    ...
    public static void showFence(Geometry geom) throws Exception {
        FenceLayerFinder fenceLayerFinder = new FenceLayerFinder(JUMPWorkbench
                .getInstance().getFrame().getContext().createPlugInContext());
    ...
    public static void activateTool(CursorTool tool) {
        PlugInContext pContext = JUMPWorkbench.getInstance().getFrame()
    ...
    public static boolean checkAllSelectedFeaturesAreInLayer(
            final String layerName) {

        Layer layer = JUMPWorkbench.getInstance().getFrame().getContext()
                .getLayerManager().getLayer(layerName);
        ...
        int numSelectedFeaturesInLayer = WorkbenchUtils.getSelectedFeatures(
                layer).size();
    ...
    public static EnableCheck createExactlyNItemsMustBeSelectedCheck(final int n) {
        return new EnableCheck() {
            ...
            public String check(JComponent component) {
                ...
                return (n != ((SelectionManagerProxy) JUMPWorkbench
                        .getInstance().getFrame().getContext().getWorkbench()
    ...
    public static boolean check(EnableCheck check) {
        ...
        if (warning != null) {
            JUMPWorkbench.getInstance().getFrame().getContext()
                    .getLayerViewPanel().getContext().warnUser(warning);
            return false;
        }
        return true;
    }
    ...
    public static Cursor getCursor(String name) {
        ...
        return Toolkit.getDefaultToolkit().createCustomCursor(
                com.vividsolutions.jump.workbench.ui.images.IconLoader.icon(
                        "Pen.gif").getImage(), new java.awt.Point(1, 31), name);
    }
    ...
    public static double viewportToMapDistance(int d) {
        double dist = -1;
        Viewport viewport = JUMPWorkbench.getInstance().getFrame().getContext()
     ...
     * @throws NoninvertibleTransformException
     */
    public static Point2D viewportToViewPoint(Coordinate modelCoordinate)
            throws NoninvertibleTransformException {
        Viewport viewport = JUMPWorkbench.getInstance().getFrame().getContext()
                .getLayerViewPanel().getViewport();
        return viewport.toViewPoint(new Point2D.Double(modelCoordinate.x,
                modelCoordinate.y));
    }
