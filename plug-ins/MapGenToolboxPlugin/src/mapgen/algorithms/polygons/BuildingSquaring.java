...
public class BuildingSquaring {
    ...
    private ArrayList mainDirectionEdges = new ArrayList();
    private ArrayList secondDirectionEdges = new ArrayList();
    private ArrayList otherDirectionEdges = new ArrayList();
    ...
    public void calculate(){
        this.outPolygon = (Polygon)this.inPolygon.clone();
    ...
    private void sortEdges(LineString ls){       
        ...
        for (int i = 0; i < coords.length-1; i++) { 
            ...
            if(deltaA < (this.angleThreshold * Math.PI /180)){
                this.mainDirectionEdges.add(new Integer(i));
            }
            else if(deltaB < (this.angleThreshold * Math.PI /180)){
                this.secondDirectionEdges.add(new Integer(i));
            }
            else{
                this.otherDirectionEdges.add(new Integer(i));
            }
        }        
    }
     ...
     */
    private boolean modifyFollowingEdges(int ringIndex, ArrayList edgeList){
        int noDeletedPoints = 0;
    
        if (edgeList.size() > 0){
	        //-- search Following points in List
	        Integer idx = (Integer)edgeList.get(0);
	        int previousIdx = idx.intValue();
