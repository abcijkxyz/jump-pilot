...
/**
 * Utility class to load data from a GeoJSON ojLayers into a layer.
 *
 * @author dcollins, modified by nicolas ribot
...
public class GeoJSONLoader {
    ...
    protected String heigthAttr = "height";
    ...
    /**
     * Create a new loader with heigth attribute forced
     *
     */
    public GeoJSONLoader(String heigthAttr) {
        this.heigthAttr = heigthAttr;
    }
    ...
    protected Renderable createPolygon(GeoJSONGeometry owner, Iterable<? extends Position> outerBoundary,
        ...
        if (positionsHaveNonzeroAltitude(outerBoundary)) {
            // Nico Ribot: if an attributes matches this height attribute, build an extruded polygon, else a normal one
            AbstractShape poly = null;
            if (properties.hasKey(this.heigthAttr) || properties.hasKey(this.heigthAttr.toUpperCase())) {
                double height = properties.hasKey(this.heigthAttr) ? (double)properties.getValue(this.heigthAttr) :
                        (double)properties.getValue(this.heigthAttr.toUpperCase());
                poly = new ExtrudedPolygon(outerBoundary);
                height = height <= 0 ? 1d : height;
//                System.out.println(height);
                ((ExtrudedPolygon)poly).setHeight(height);
                poly.setAttributes(attrs);
                if (innerBoundaries != null) {
                    for (Iterable<? extends Position> iter : innerBoundaries) {
                        ((ExtrudedPolygon)poly).addInnerBoundary(iter);
                    }
                }
            } else {
                poly = new Polygon(outerBoundary);

                poly.setAttributes(attrs);

                if (innerBoundaries != null) {
                    for (Iterable<? extends Position> iter : innerBoundaries) {
                        ((Polygon)poly).addInnerBoundary(iter);
                    }
                }
            }
