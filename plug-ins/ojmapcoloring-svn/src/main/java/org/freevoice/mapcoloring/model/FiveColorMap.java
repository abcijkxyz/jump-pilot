...
public class FiveColorMap<V>
{
   ...
   public FeatureCollection colorMap(Layer selectedLayer, boolean bufferForIntersection)
   {
      ...
      List<Feature> featureList = (List<Feature>) selectedLayer.getFeatureCollectionWrapper().getFeatures();
      ...
      SimpleGraph<Integer, String> mapGraph = new SimpleGraph<Integer, String>(String.class);
      ...
      Map<Integer, Set<Integer>> revIntersections = new HashMap<Integer, Set<Integer>>();
      ...
      for (int i = 0; i < numFeatures && !isCancelled; i++)
      {
         ...
         //feature
         List maybeIntersecting = spatIdx.query(bufferedGeometry.getEnvelopeInternal());
         ...
         for (Feature maybeIntersects : (List<Feature>) maybeIntersecting)
         {
            ...
            if (intersects)
            {
               ...
               if (revNeighbor == null)
               {
                  revNeighbor = new HashSet<Integer>();
      ...
      ColorGraph<Integer> innerColorMap = new ColorGraph<Integer>();
   ...
   {
      FeatureSchema newSchema = (FeatureSchema) selectedLayer.getFeatureCollectionWrapper().getFeatureSchema().clone();
