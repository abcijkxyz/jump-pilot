...
public class ColorGraph<V>
{
   ...
   private Stack<V> removedVertices = new Stack<V>();
    ...
    * not be in this map if the graph couldn't be properly colored, e.g. the graph was not planer.
    */
   public Map<V, String> colorGraph(SimpleGraph<V, String> graph)
   {
      Map<V, Set<V>> neighborMap = new HashMap<V, Set<V>>();
      //copy all edges before we decompose the graph
      NeighborIndex neighborIndex = new NeighborIndex(graph);
      for (V s : graph.vertexSet())
      {
         neighborMap.put(s, new HashSet<V>(neighborIndex.neighborsOf(s)));
      }
   ...
   {
      SimpleGraph<ColoredVertex, ColoredVertex> graph = new SimpleGraph<ColoredVertex, ColoredVertex>(ColoredVertex.class);

      Map<V, ColoredVertex> addedVertices = new HashMap<V, ColoredVertex>();
      ...
      while (! removedVertices.empty())
      {
         ...
         {
            ColoredVertex addedEdge = addedVertices.get(neighbor);
         ...
         ColoredVertex<V> newVertex = new ColoredVertex<V>(vertex);
      ...
      Set<ColoredVertex> coloredSet = graph.vertexSet();

      Map<V, String> colorMap = new HashMap<V, String>(coloredSet.size());
   ...
   private void decomposeGraph()
   {
      ...
      Set<V> originalVertices = new HashSet<V>(workingGraph.vertexSet());

      Set<V> degreeOfFiveSet = new HashSet<V>();

      NeighborIndex<V, String> neigborIndex = new NeighborIndex<V, String>(workingGraph);
   ...
   private boolean handleGraphWithOnlyDegreeOfFive(SimpleGraph<V, String> graph,
                                                   Set<V> degreeOfFive, NeighborIndex<V, String> neigborIndex)
   {
      V n1 = null;
      V n2 = null;
      boolean decomposed = false;
